...
const terbilangSatuSatu = (angka: any) => {
  return angka
    .split("")
    .map((item: any) => (item == 0 ? "nol" : numberToText(item)))
    .join(" ");
};

export function angkaTerbilang(target: any, settings = { decimal: "." }) {
  if (typeof target !== "string") target = String(target);
  if (target.indexOf(settings.decimal) > -1) {
    /* Dengan Desimal */
    target = target.split(settings.decimal);
    return `${terbilang(target[0])} koma ${terbilangSatuSatu(target[1])} rupiah`;
  } else {
    /* Tanpa Desimal */
    return `${terbilang(target)} rupiah`;
  }
}
