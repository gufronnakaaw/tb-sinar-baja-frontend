...
// utils
import { ListProdukCashier } from "@/pages/cashier/menu/selling";
import { ProdukSearchType } from "@/types/products.type";
import { formatRupiah } from "@/utils/formatRupiah";

type CardSellingProductProps = {
  item: ProdukSearchType;
  setListProduk: React.Dispatch<React.SetStateAction<ListProdukCashier[]>>;
  field: string | undefined;
};

export default function CardSellingCashier({
  setListProduk,
  field,
  item,
  ...
  return (
    <div className="grid grid-cols-[1fr_50px] items-center rounded-xl border border-default-300 p-4 transition hover:border-rose-500">
      <div className="grid w-3/4 gap-[2px]">
        ...
        {item.gudang?.length ? (
          ...
          <div className="grid grid-cols-[110px_6px_1fr] gap-1 text-sm text-black">
            ...
            <div className="font-medium">:</div>
            <p className="font-bold text-rose-500">Tidak Ada</p>
          </div>
        )}
      </div>
